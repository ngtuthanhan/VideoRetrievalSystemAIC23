...
all_map_keyframes = glob.glob(os.path.join(root_data,'data-batch-1','map-keyframes', '*.csv'))
keyframe_json = []
...
for map_keyframes in tqdm(all_map_keyframes[::-1]):
    ...
    for path in all_keyframes_in_this_video:
        ...
        if os.path.exists(f'./data/color_vector_full/{keyframe}.npy'):
            continue
        
        mean_hist_feature = mean_color_vector(path)

        with open(f'./data/color_vector_full/{keyframe}.npy','wb') as f:
